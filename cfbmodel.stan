  ...
  //hard coding prior coefficients for speed
  playerPriorMean = ((-0.0733 + QBind*0.02422 + recGrade*0.000524 - recGradeUse*0.036944) + //recruiting information
                    plysLastYr .* pmLastYr ) ./     //last season weighted by plays
                    (plysLastYr + 1 ) ; //dividing by the weights
  ...
  b ~ student_t(8, playerPriorMean, sqrt(s2player)* positionVarCoef ./ lastYrWeight   );
  //b~normal(playerPriorMean, positionVarCoef * sqrt(s2player));
  
  
  s2player ~ gamma(0.5 , 90); // 
  ...
  // prior for play type 
  for(i in 1:nPlayType){
  g[i] ~ normal(0, sqrt(s2playType));
  }
  s2playType ~ gamma(100, 1000);
  ...
  mu[i] = eta[i] + home[i]*u + g[playType[i]]    ; // 
